<?php
    ...
    if (isset($_REQUEST["search"])) {
        $con = $p->getsearchnvxoa($_REQUEST["txtsearch"], $_REQUEST["txtdanhmuc"]);
        ...
        if (!$con || $con->num_rows == 0) {
            ...
            echo "<script>window.location.href = 'quanly.php?dsxoanv';</script>";
            exit(); // Dừng script sau khi chuyển hướng
        }
    } else {
        $con = $p->getallnvxoa(); // Lấy tất cả nhân viên bị xóa
    }
...
<?php
    ...
    echo "<a class='add-button' href='?dsnv'>Quay Lại</a>";
    echo "</div>";
    echo "<article id='a3'>";
    echo "<table id='a30'>";
    echo "<tr>";
    ...
    if (!$con || $con->num_rows == 0) {
        ...
        while ($r = $con->fetch_assoc()) {
            ...
            echo "<td>" . $r["TenCV"] . "</td>";
            echo "<td>
                    <a class='edit-button' href='?kpnv=" . $r["MaNV"] . "'onclick='return confirm(\"Bạn có muốn khôi phục nhân viên này?\")'>Khôi Phục</a> 
                </td>";
            echo "</tr>";
        }
    }
...
.study-info {
    ...
    margin-left: 40px; /* Tạo khoảng cách từ lề trái */
