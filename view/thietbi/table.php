<?php
    ...
    // Xử lý tìm kiếm hoặc hiển thị danh sách
    if (isset($_REQUEST["search"])) {
        // Gọi phương thức tìm kiếm
        $con = $p->getsearchtb($_REQUEST["txtsearch"]);
        $t = $_REQUEST["txtsearch"];

        // Kiểm tra kết quả trả về từ getsearchtb
...
<!-- Form tìm kiếm và nút thêm thiết bị -->
...
<?php
    ...
    if ($con) {
        ...
        echo " <a class='add-button1' href='?dsxoatb'>Danh Sách Xóa Thiết Bị</a>";
        echo "<a class='add-button2' href='?dstbsua'>Danh Sách Thiết Bị Cần Sửa</a>";
        echo "</div>";
        echo "<article id='a3'>";
        echo "<table id='a30'>";
        echo "<tr>";
        ...
        while ($r = $con->fetch_assoc()) {
            echo "<tr>";
            echo "<td>" . $stt++ . "</td>";
            echo "<td><a href='?ctnv=" . $r["MaTB"] . "'>" . $r["TenTB"] . "</a></td>";
            echo "<td>" . $r["NoiSanXuat"] . "</td>";
            echo "<td>" . $r["TinhTrang"] . "</td>";
            echo "<td>
                    <a class='edit-button' href='?stb=" . $r["MaTB"] . "'>sửa</a> 
                    <a class='delete-button' href='?xoatb=" . $r["MaTB"] . "' onclick='return confirm(\"Bạn có muốn xóa thiết bị này?\")'>xóa</a>
                </td>";
            echo "</tr>";
        }
