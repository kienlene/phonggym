...
<?php
    ...
    $p = new c_kythuatvien();
    $con = $p->get_dstb_chuaduyet("Chưa duyệt");
?>
<body>
    <h2>Danh sách phiếu tình trạng cần duyệt</h2>
    <table>
        <thead>
            <tr>
                <th>STT</th>
        ...
        <tbody>
            ...
            if($con){
                ...
                while ($r = $con->fetch_assoc()) {
                    ...
                    echo '<td>
                        <form method="post">
                        <button class="btn btn-blue" type="submit" name="btnduyet" value="'.$r["MaPhieu"].'">
                            Duyệt
                        </button>
                        </form>
                    </td>';
                ...
                echo "<script>alert('Không có phiếu chờ duyệt!');</script>";
...
if (isset($_POST["btnduyet"])) {
    $maphieu = $_POST["btnduyet"];
    // $trangthai = $_POST["trangthai"];
        $result = $p->update_trangthai($maphieu, "Duyệt");

        if ($result) {
            echo "<script>alert('Cập nhật trạng thái thành công!');</script>";
            echo '<script>window.location.href="quanly.php?dstbsua"</script>';
        } else {
